...
export default function ContactProfile() {
  ...
  const [manualAmount, setManualAmount] = useState(false); // ✅ override flag
  ...
  // جلب المنتجات كلها علشان نعرضها في القائمة لما نعمل edit
  ...
  // كل ما تتغير المنتجات → نحسب amount
  useEffect(() => {
    if (!manualAmount) {
    ...
    {contact.assignedTo && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-4">
          ...
          {contact.stage === "sales" && (
            <div className="card">
              <h3 className="font-bold mb-3">Sales Deal</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                ...
                <div className="field">
                  ...
                  <input
                    ...
                    value={form.amount}
                    onChange={(e) => {
                      setManualAmount(true); // ✅ أي إدخال يدوي يبقى override
                      handleChange(e);
                    }}
                    className="input"
                  />
                    ...
                    <tr key={i}>
                      ...
                      <td>{editProducts ? (
                        <input type="number" min="1" value={line.qty}
                          onChange={(e) => handleProductChange(i, "qty", Number(e.target.value))} />
                      ) : (
                        <span>{line.qty}</span>
                      )}</td>
                      <td>{editProducts ? (
                      ...
                      <td>{editProducts && (
                        <button onClick={() => removeProductLine(i)}>✕</button>
                      )}</td>
                    </tr>
                  ))}
                </tbody>
              ...
              {editProducts && (
                <button onClick={addProductLine} className="btn mt-2">+ Add Product</button>
              )}
              <div className="mt-2">
                <button onClick={() => setEditProducts(!editProducts)} className="btn ghost">
                  {editProducts ? "Done" : "Edit Products"}
                </button>
              </div>
            </div>
          )}
        ...
        <div className="space-y-4">
          <div className="card">
            ...
            <div className="flex flex-col gap-2">
              {contact.stage === "lead" && <button onClick={() => handleStageChange("customer")} disabled={busy} className="btn">Convert to Customer</button>}
              {contact.stage === "customer" && <button onClick={() => handleStageChange("sales")} disabled={busy} className="btn">Move to Sales</button>}
              {(contact.stage === "customer" || contact.stage === "sales") && <button onClick={() => handleStageChange("lead")} disabled={busy} className="btn ghost">Revert to Lead</button>}
            </div>
          </div>
      ...
      <div className="mt-6">
        <MessageList contactId={id} tenantId={contact.tenantId} />      </div>
    </div>
  );
}
